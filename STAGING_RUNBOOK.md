...
Record release/image digest, prompt version, scenario identity, approximate test time, and call ID. Verify qualification fields, deterministic routing decision, completed handoff or callback, and persisted final outcome. Transfer requests now stay pending until signed callback evidence establishes their result. Completed Dial evidence must also confirm a bridge before handoff is recorded. Failure and uncertain requests create durable recovery work; stale unresolved attempts can be reconciled with the bounded command in [the live-call guide](NATIVE_AUTH_AND_LIVE_VALIDATION.md). Verify these paths with real controlled calls before promotion. A confirmed bridge can still be voicemail, so human-answer acceptance requires an independent coordinator observation. For callbacks, verify consent and SMS outcome independently. Inspect the Operations checks and call details. Confirm that latency sample coverage is explicit and missing measurements remain unknown. Playback acknowledgement is not audible-onset latency; do not add the stage columns into an end-to-end total.
